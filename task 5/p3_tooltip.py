...
for airport in dataset['nodes']:
    ...
    G.add_node(airport_code, importance=importance_factor, latitude=airport['latitude'], longitude=airport['longitude'],
               id=airport['id'], name=airport['name'], city=airport['city'], country=airport['country'],
               posx=airport['posx'], posy=airport['posy'], state=airport['state'])
...
def update_graph(min_importance, min_weight):
    ...
    labels = {node: f"Name: {data['name']}\nCity: {data['city']}\nCountry: {data['country']}" for node, data in
              filtered_graph.nodes(data=True)}
    nx.draw_networkx_labels(filtered_graph, pos, labels, font_size=8)

    plt.title('Tooltip Visualization')
    plt.axis('off')
    plt.show()
