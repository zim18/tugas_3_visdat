...
# Define edge tooltips
edge_tooltips = {}
for edge in G.edges():
    source = edge[0]
    target = edge[1]
    weight = G.get_edge_data(source, target)['weight']
    edge_tooltips[edge] = f"Weight: {weight}"

# Visualize the graph with tooltips and interactive legend
fig, ax = plt.subplots(figsize=(12, 8))
pos = nx.spring_layout(G)
nx.draw_networkx_nodes(G, pos, node_color='lightblue', node_size=1000)
nx.draw_networkx_edges(G, pos, width=0.5, alpha=0.5, edge_color='gray')
nx.draw_networkx_labels(G, pos, labels={node:G.nodes[node]['city'] for node in G.nodes()}, font_size=8)
...
# Add tooltips and interactive legend to the plot
tooltip_node = mpld3.plugins.PointHTMLTooltip(nx.draw_networkx_nodes(G, pos), labels=node_tooltips, voffset=10, hoffset=10)
tooltip_edge = mpld3
